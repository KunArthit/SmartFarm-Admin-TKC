		...
		const fetchTrackingStatus = async () => {
			...
			try {
				...
				if (items) {
					...
					setError(t('No tracking data found for this barcode.'));
				}
			} catch (err) {
				setError(t('Failed to fetch tracking status. Please try again.'));
			} finally {
				setLoading(false);
			}
		};
	...
	return (
		<Box>
			{loading && (
				...
				>
					<CircularProgress />
					<Typography marginLeft={2}>{t('Loading...')}</Typography>
				</Box>
			)}
