...
function IssuesWidget({ title: propTitle }: IssuesWidgetProps) {
	...
	useEffect(() => {
		axios
			.get(`${API_BASE_URL}/reports/solution-count`)
			.then((res) => {
				setTotalSolutions(res.data.totalSolutions);
				setClosedToday(res.data.closedToday);
				setIsLoading(false);
			})
			.catch(() => {
				setIsLoading(false);
	...
	return (
		<Paper className='flex flex-col flex-auto shadow rounded-xl overflow-hidden'>
			...
			<div className='text-center mt-16'>
				...
				<Typography className='text-lg font-medium text-amber-600'>{t('Total Solutions')}</Typography>
			</div>
			<Typography
				className='flex items-baseline justify-center w-full mt-20 mb-24 space-x-8'
				color='text.secondary'
			>
				<span className='truncate'>{t('Closed Today')}:</span>
				<b>{closedToday}</b>
			</Typography>
		</Paper>
	);
