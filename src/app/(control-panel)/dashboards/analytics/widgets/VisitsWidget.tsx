...
/**
 * Visits widget - Unique Customers.
 */
function Visits() {
	...
	// Process order data for the last 30 days (Unique Customers-based)
	const processOrderData = (orders: OrderData[]): VisitsData => {
		const now = new Date();
		...
		// Group unique customers by day for the last 30 days
		const customersByDay: Record<string, Set<number>> = {};
		const labels: string[] = [];
		...
		for (let i = 29; i >= 0; i--) {
			...
			customersByDay[dateKey] = new Set<number>();
			labels.push(labelKey);
		}

		// Fill in actual customer data (unique user_id per day)
		last30DaysOrders.forEach((order) => {
			const orderDate = new Date(order.created_at);
			const dateKey = orderDate.toISOString().split('T')[0];

			if (customersByDay[dateKey]) {
				customersByDay[dateKey].add(order.user_id);
			}
		});

		// Create series data (unique customers per day)
		Object.keys(customersByDay)
			.sort()
			.forEach((dateKey) => {
				seriesData.push(customersByDay[dateKey].size);
			});

		// Calculate total unique customers and change percentage
		const currentUniqueCustomers = new Set(last30DaysOrders.map((order) => order.user_id)).size;
		const previousUniqueCustomers = new Set(previous30DaysOrders.map((order) => order.user_id)).size;
		...
		return {
			...
			amount: currentUniqueCustomers,
			labels: labels,
			previousAmount: previousUniqueCustomers,
